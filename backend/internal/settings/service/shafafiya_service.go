...
func (s *ShafafiyaService) CreateShafafiyaSettings(ctx context.Context, settings domain.ShafafiyaOrgSettings) (domain.ShafafiyaOrgSettings, error) {
	...
	org, err := s.orgRepo.GetByID(ctx, settings.OrganizationID)
	if err != nil {
...
// UpdateShafafiyaSettings updates all Shafafiya settings for an organization
func (s *ShafafiyaService) UpdateShafafiyaSettings(ctx context.Context, orgID uuid.UUID, settings domain.ShafafiyaOrgSettings) (*domain.ShafafiyaOrgSettings, error) {
	if orgID == uuid.Nil {
		return nil, domain.NewDomainError("organization ID is required", domain.ErrShafafiyaNotFound)
	}

	// Verify settings exist
	existing, err := s.repo.GetShafafiyaSettings(ctx, orgID)
	if err != nil {
		return nil, fmt.Errorf("shafafiya settings not found: %w", err)
	}
	...
	existing.ProviderCode = settings.ProviderCode
	existing.DefaultCurrencyCode = settings.DefaultCurrencyCode
	existing.DefaultLanguage = settings.DefaultLanguage
	existing.IncludeSensitiveData = settings.IncludeSensitiveData
	existing.CostingMethod = settings.CostingMethod
	existing.AllocationMethod = settings.AllocationMethod

	// Update in repository
	updated, err := s.repo.UpdateShafafiyaSettings(ctx, orgID, existing)
	if err != nil {
...
// ListAllShafafiyaSettings retrieves all Shafafiya configurations (admin only)
func (s *ShafafiyaService) ListAllShafafiyaSettings(ctx context.Context) ([]domain.ShafafiyaOrgSettings, error) {
	settings, err := s.repo.List(ctx, 1000, 0)
	if err != nil {
